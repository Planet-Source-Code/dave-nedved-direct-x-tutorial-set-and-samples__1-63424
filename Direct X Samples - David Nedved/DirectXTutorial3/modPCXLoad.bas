...
Rem // | Author:         (PCX-Loading): ALKO            |
Rem // | Email:          alfred.koppold@freenet.de      |
Rem // |                                                |
Rem // --------------------------------------------------
...
Open FileName For Binary Lock Write As #nFreefile
    ...
    Rem // Get palette indication byte
...
If Header.Bpp = 8 Then
    If Header.Planes = 1 Then
        ...
        Set LoadPCX = D3DX.CreateTexture(D3DDevice, nWidth, nHeight, D3DX_DEFAULT, 0, D3DFMT_X8R8G8B8, D3DPOOL_MANAGED)
        
        Dim d3dsd As D3DSURFACE_DESC
        LoadPCX.GetLevelDesc 0, d3dsd
        
        Rem // Scale only if necesarry
        ...
        Rem // The mipmap chain was created by d3d, but d3d created "dirty" mipmaps,
        Rem // lets tell D3D to use an filter for this texture
...
Rem // Creates an blury, but scaled pcx wich you could use for an texture
Rem // This should do something like linear filtering, I dunno if I got it working right :/
   ...
   For j = 0 To Scale_Height - 1
      ...
      For i = 0 To Scale_Width
        ...
        PixData((j * Scale_Width) + i).Blue = CheckByte( _
         ...
         Rem // FOR DEBUG STUFF
         Rem // Create an picturebox on frmMain to see the scaled texture
         Rem // frmMain.Picture2.ForeColor = RGB(PixData((j * Scale_Width) + i).Red, PixData((j * Scale_Width) + i).Green, PixData((j * Scale_Width) + i).Blue)
         Rem // frmMain.Picture2.PSet (i, j)
...
DXCopyMemory Übergabe(0), ImageArray(0), UBound(ImageArray) + 1

ReDim ImageArray(GrößeBMP)
For i = 0 To BytesLine * Lines - BytesLine Step BytesLine
    DXCopyMemory ImageArray(Standort), Übergabe(Größe - i - BytesLine), BytesLine
